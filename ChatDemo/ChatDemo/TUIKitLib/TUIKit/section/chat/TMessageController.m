...
- (void) setupTableView {
    [self.tableView registerNib:[UINib nibWithNibName:@"ChatHouseInfoCell" bundle:nil] forCellReuseIdentifier:@"ChatHouseInfoCell"];
}
...
- (NSMutableArray *)transUIMsgFromIMMsg:(NSArray *)msgs
{
    ...
    for (NSInteger k = msgs.count - 1; k >= 0; --k) {
        ...
        if (!msg.isSelf) {
            NSLog(@"昵称 %@", [msg getSenderProfile].nickname);
            NSLog(@"头像 %@", [msg getSenderProfile].faceURL);
            NSLog(@"dict %@", [msg getSenderProfile].customInfo);
        }
        ...
        for (int i = 0; i < msg.elemCount; ++i) {
            ...
            if([elem isKindOfClass:[TIMTextElem class]]){
                ...
                if (custom.data) {
                    NSDictionary *dict = [TIMCustomElemTool parsingData:custom.data];
                    NSDictionary *params = dict[ACTION_PRAMS];
                    if ([dict[USER_ACTION] isEqualToString:HOUSE_INFO]){ // 房源信息
                        THouseInfoCellData *houseInfoData = [THouseInfoCellData new];
                        houseInfoData.title = params[@"title"];
                        houseInfoData.imgURL = params[@"imgURL"];
                        data = houseInfoData;
                    } else {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else if ([data isKindOfClass:[THouseInfoCellData class]]) {
        ChatHouseInfoCell *cell = [[ChatHouseInfoCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"ChatHouseInfoCell"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if([data isKindOfClass:[TTextMessageCellData class]]){
        ...
        cell = [tableView dequeueReusableCellWithIdentifier:@"ChatHouseInfoCell"];
        if (!cell) {
            cell = [[NSBundle mainBundle] loadNibNamed:@"ChatHouseInfoCell" owner:nil options:nil].firstObject;
        }
    }
...
- (TIMMessage *)transIMMsgFromUIMsg:(TMessageCellData *)data
{
    ...
    if([data isKindOfClass:[TTextMessageCellData class]]){
        ...
        [msg addElem:imFile];
    } else if ([data isKindOfClass:[THouseInfoCellData class]]) { // 自定义
        THouseInfoCellData *tempHouseInfo = (THouseInfoCellData *)data;
        TIMCustomElem *cus_elem = [TIMCustomElem new];
        NSMutableDictionary *dictionary = [NSMutableDictionary dictionary];
        dictionary[@"title"] = tempHouseInfo.title;
        dictionary[@"imgURL"] = tempHouseInfo.imgURL;
        cus_elem.data = [TIMCustomElemTool packToSendData:HOUSE_INFO params:dictionary];
        NSInteger index = [msg addElem:cus_elem];
#warning 自定义消息 index为0，才是添加成功
        NSLog(@"%d", index);
    }
    data.custom = msg;
    return msg;
...
- (void)sendHouseInfoMessage:(id)houseInfo {
    ...
    houseInfoData.title = params[@"title"];
    houseInfoData.imgURL = params[@"imgURL"];
    [self sendMessage:houseInfoData];
}
