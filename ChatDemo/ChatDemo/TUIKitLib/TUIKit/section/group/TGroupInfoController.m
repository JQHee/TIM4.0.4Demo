    ...
    [[TIMGroupManager sharedInstance] getGroupInfo:@[_groupId] succ:^(NSArray *arr) {
        if(arr.count == 1){
            ...
            [[TIMGroupManager sharedInstance] getGroupMembers:ws.groupId succ:^(NSArray *members) {
                ...
                TSwitchCellData *switchData = [[TSwitchCellData alloc] init];
                switchData.isOn = YES;
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else if([data isKindOfClass:[TSwitchCellData class]]){
        return [TSwitchCell getHeight];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if([data isKindOfClass:[TGroupCommonCellData class]]){
        ...
        selector = ((TSwitchCellData *)data).selector;
    }
    if(selector){
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if([data isKindOfClass:[TGroupCommonCellData class]]){
        ...
        TSwitchCell *cell = [tableView dequeueReusableCellWithIdentifier:TSwitchCell_ReuseId];
        if(!cell){
            cell = [[TSwitchCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:TSwitchCell_ReuseId];
        }
        [cell setData:(TSwitchCellData *)data];
        return cell;
    }
    else if([data isKindOfClass:[TButtonCellData class]]){
