...
@implementation TAddMemberResult
@end

@interface TAddMemberController () <UITableViewDelegate, UITableViewDataSource, TAddIndexViewDelegate, TAddIndexViewDataSource, UITextFieldDelegate, UICollectionViewDelegate, UICollectionViewDataSource>
@property (nonatomic, strong) NSMutableArray *formatData;
@property (nonatomic, strong) NSMutableArray *selectedData;
@property (nonatomic, strong) UIButton *rightButton;
@end

@implementation TAddMemberController
...
{
    self.title = @"添加成员";
    self.parentViewController.title = @"添加成员";
    self.view.backgroundColor = TAddMemberController_Background_Color;
...
//    _rightButton.enabled = NO;
    ...
    CGFloat textOriginX = _collectionView.frame.origin.x + _collectionView.frame.size.width + TAddMemberController_Margin;
    ...
    UIView *line = [[UIView alloc] initWithFrame:CGRectMake(0, _textField.frame.origin.y + _textField.frame.size.height, self.view.frame.size.width, TAddMemberController_Line_Height)];
    line.backgroundColor = TAddMemberController_Line_Color;
...
- (void)setupData
{
    ...
    for (TAddCellData *user in _users) {
        [allUsers setObject:user forKey:user.identifier];
    }
    ...
    [[TIMGroupManager sharedInstance] getGroupMembers:_groupId succ:^(NSArray *members) {
        for (TIMGroupMemberInfo *member in members) {
            TAddCellData *user = [allUsers objectForKey:member.member];
            if(user){
                user.state = TAddCell_State_Solid;
            }
            else{
        ...
        ws.formatData = [TAddHelper arrayWithFirstLetterFormat:allUsers.allValues];
...
- (void)leftBarButtonClick:(UIButton *)sender
{
    ...
    if(_delegate && [_delegate respondsToSelector:@selector(didCancelInAddMemberController:)]){
        [_delegate didCancelInAddMemberController:self];
    }
}
...
- (void)rightBarButtonClick:(UIButton *)sender
{
    ...
    if(![_textField.text isEqualToString:@""]){
        [_textField resignFirstResponder];
        [members addObject:_textField.text];
    }
    else{
    ...
    [[TIMGroupManager sharedInstance] inviteGroupMember:_groupId members:members succ:^(NSArray *members) {
        if(ws.delegate && [ws.delegate respondsToSelector:@selector(addMemberController:didAddMemberResult:)]){
            NSMutableArray *addArray = [NSMutableArray array];
            for (TIMGroupMemberResult *member in members) {
                if(member.status == TIM_GROUP_MEMBER_STATUS_SUCC){
                    [addArray addObject:member.member];
                }
            }
            TAddMemberResult *result = [[TAddMemberResult alloc] init];
            result.groupId = ws.groupId;
            result.code = 0;
            result.desc = 0;
            result.addMembers = addArray;
            [ws.delegate addMemberController:ws didAddMemberResult:result];
        }
    } fail:^(int code, NSString *msg) {
        if(ws.delegate && [ws.delegate respondsToSelector:@selector(addMemberController:didAddMemberResult:)]){
            TAddMemberResult *result = [[TAddMemberResult alloc] init];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(width > Screen_Width * 0.6){
        ...
        [UIView animateWithDuration:0.3 animations:^{
            ...
            CGFloat originX = ws.collectionView.frame.origin.x + ws.collectionView.frame.size.width + TAddMemberController_Margin;
